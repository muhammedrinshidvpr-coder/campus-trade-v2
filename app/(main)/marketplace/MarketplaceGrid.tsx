...
export default function MarketplaceGrid({ items }: { items: Item[] }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-4 backdrop-blur-xl flex flex-col md:flex-row gap-4 items-center justify-between shadow-xl">
        {/* Search Input */}
        <div className="relative w-full md:w-1/2">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-zinc-900/50 border border-white/10 rounded-xl text-white outline-none focus:ring-2 focus:ring-blue-500 transition-all placeholder:text-slate-500"
          />
        </div>
          ...
          >
            ...
            <div
              className={`
                ...
                ${hoveredCard === item.id ? "shadow-2xl shadow-blue-500/20 scale-[1.02] border-blue-500/30" : "shadow-lg"}
              `}
            >
              {/* Item Image */}
              <div className="relative w-full h-48 bg-zinc-800 flex items-center justify-center overflow-hidden shrink-0">
                {item.image_url ? (
                  <img
                    src={item.image_url}
                    alt={item.title}
                ...
                <div className="absolute top-3 right-3 z-20 px-3 py-1 bg-black/60 backdrop-blur-md rounded-full text-xs text-white font-medium border border-white/10">
              ...
              <div className="p-5 flex flex-col flex-grow">
                {/* Category Tag */}
                {item.category && (
                  <span className="inline-block px-2 py-1 bg-white/10 text-slate-300 text-xs rounded-md mb-2 w-max">
                    {item.category}
                  </span>
                )}
                ...
                {/* ACTION BUTTONS (Updated with Share Button) */}
                <div className="flex flex-col space-y-2 mt-auto">
                  {/* Row for Contact & Share */}
                  <div className="flex gap-2 w-full">
                    {/* Contact Button */}
                    <button
                      onClick={() => handleContactSeller(item)}
                      className="flex-1 py-2.5 px-4 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white font-semibold rounded-lg transition-all duration-200 flex items-center justify-center space-x-2 shadow-lg shadow-blue-500/25 group-hover:shadow-blue-500/40"
                    >
                      <svg
                        className="w-5 h-5"
                        fill="currentColor"
                    ...
                    <button
                      onClick={(e) => {
                        ...
                        const text = `Hey! I found "${item.title}" for ₹${item.price} on CampusTrade 🚀\n\nCheck it out here: ${itemUrl}`;
                        window.open(
                          `https://wa.me/?text=${encodeURIComponent(text)}`,
                          "_blank",
                        );
                      }}
                      className="px-4 py-2.5 bg-[#25D366]/10 hover:bg-[#25D366]/20 text-[#25D366] rounded-lg border border-[#25D366]/20 transition-all flex items-center justify-center shrink-0"
